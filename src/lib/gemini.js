// Gemini API client — all real API calls live here.
...
// #12 Clean user-facing error, detailed info to console only
export async function geminiPostWithRotation(path, settings, body) {
  ...
  // Log details to console, show clean message to UI
  console.error("All Gemini API keys failed:\n" + errors.join("\n"));
  throw new Error("All API keys failed. Check your keys in Settings → General.");
}
...
export async function streamFetchWithRotation(path, settings, body) {
  ...
  console.error("All Gemini API keys failed (stream):\n" + errors.join("\n"));
  throw new Error("All API keys failed. Check your keys in Settings → General.");
}
...
export async function extractText(file) {
  ...
  return `[Unsupported format: ${file.name}. Upload a PDF, DOCX, or TXT for full RAG support.]`;
}
...
export async function ragAnswer({ question, chunks, settings, apiKey, model = "gemini-2.5-flash-lite", onChunk }) {
  ...
  const systemPrompt = `You are a study assistant. Answer questions using the provided document excerpts.

RULES:
1. If the answer is in the documents, answer from them and cite sources (e.g. "According to Source 2...").
2. If partially covered, answer what the docs cover, then add supplemental info prefixed with "Note: the following is general knowledge, not from your documents:".
3. If not in the docs at all, say "This topic does not appear in your uploaded documents." then provide a helpful general answer prefixed with "General knowledge (not from your documents):".
...
${context}`;
  ...
  const body = {
    ...
    generationConfig: { temperature: settings?.temperature ?? 0.2, maxOutputTokens: 2048 },
  };

  const r = await streamFetchWithRotation(`/models/${model}:streamGenerateContent?alt=sse`, _settings, body);
  return _readSSEStream(r, onChunk);
}
...
async function _readSSEStream(r, onChunk) {
    ...
    try {
      const data = await geminiPostWithRotation(`/models/${model}:generateContent`, settings, {
        contents: [{ role: "user", parts: [{ text:
          `Analyze these excerpts and return JSON with "topic" (main academic subject) and "keywords" (8-12 specific content keywords, NOT boilerplate words like copyright/author/page/chapter).
...
Reply ONLY with the JSON object, no markdown.` }] }],
    ...
    if (!topicKeywords.length) { filtered.push(...chunks); continue; }
    ...
    chunks.forEach(chunk => {
      ...
      const isSubstantive = chunk.text.trim().length > 120;
      if (hasKeyword || isSubstantive) filtered.push(chunk);
    });
  }
...
export async function generateQuiz({ chunks, config, settings, apiKey, model = "gemini-2.5-flash-lite", onStep }) {
  const _settings = settings || { apiKeys:[{key:apiKey}], activeKeyIndex:0 };

  onStep?.("Filtering content chunks…");
  const topicFiltered = await filterTopicChunks(chunks, _settings, model).catch(() => chunks);

  onStep?.("Building context…");
  ...
  const timerNote = withTimer ? `\nAlso add "suggestedSeconds": <integer> to every question.` : "";

  const prompt = `You are a rigorous academic quiz generator.

Generate exactly ${config.numQ} quiz questions based ONLY on the document excerpts at the bottom.
...
${context}`;
  ...
  throw new Error("Quiz generation failed to produce valid JSON. Try fewer questions or a single question type.");
}
...
Respond ONLY with JSON:
{
  "score": 0-100,
  "correct": true/false,
  "feedback": "detailed, honest feedback",
...
export async function studyChat({ messages, chunks, settings, apiKey, model = "gemini-2.5-flash-lite", onChunk }) {
  ...
  const system = `You are a helpful, precise study assistant. Answer using the provided document excerpts.

RULES:
1. If the answer is clearly in the documents, answer from them and cite sources.
2. If partially in the documents, answer what you can and prefix supplemental info with "Note: the following is based on general knowledge, not your uploaded documents:".
3. If not in the documents at all, say "This topic does not appear in your uploaded documents." then provide a general answer prefixed with "General knowledge (not from your documents):".
4. Never silently mix document content with outside knowledge.

After your answer, output ALL document sources you drew from, one per line:
SOURCES_JSON:{"docName":"filename.pdf","page":1,"snippet":"first 80 chars of relevant passage"}
...
export function parseSourcesFromReply(text) {
  ...
  while ((match = regex.exec(text)) !== null) {
    try {
      ...
      if (!sources.find(s => `${s.docName}||${s.page}` === key)) sources.push(parsed);
    } catch {}
  }
